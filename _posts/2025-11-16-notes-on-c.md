...
So that's the background of today's post: some things in C that I noticed when I saw a C programme (see previous post on generating  dummy data). Absolutely obvious stuff to people who are actual computer scientists.
...
Of course. How else would you start a blog themed "stuff in C I didn't understand" if it wasn't with damn POINTERS. This thing that took me a loooong while to understand, but after a couple of attempts it finally clicked (I think). 
...
"Well if b is just a local definition of the variable, then if I do"
...
Because once we no longer need that variable, we can get rid of it. 
