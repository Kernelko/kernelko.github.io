---
layout: default
title: lzrdtrak - How are we going to store data
---

# Background

See the previous post on the actual background of this project. Here we are just handling the data collection and storage.
There will be a lot of data. Roughly, same format. 

The data we store will be:

- stored every minute/5 seconds ? Not decided yet how often
- will store the timestamp
- will store the entry type: humidity or temperature
- will store the entry location: probably will be something like hot corner, cold corner, hide 1, hide 2 etc.
...
4) Deploy the interface to raspberry pi for now, make it accessible on LAN, make sure server restarts on the restart of the rapi
